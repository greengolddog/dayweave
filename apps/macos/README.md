...
writable Google Calendar. The macOS client invokes that contract from the
generated-schedule publication review, with an encrypted exact recovery journal
covering preview, one-shot approval, enqueue, and status polling. The Inbox
action above remains the separate single-item event/task publication path.
Published elapsed events are immutable Calendar history and are never rewritten,
deleted, or reused. Automatic firm-horizon rolling/publication and inbound
interpretation of Google edits, moves, or deletions remain future
client/integration work. Tentative blocks remain app-only and are never accepted
by this server path. Enabling the publisher therefore does not publish a
schedule without explicit review or complete `SCH-006`.
