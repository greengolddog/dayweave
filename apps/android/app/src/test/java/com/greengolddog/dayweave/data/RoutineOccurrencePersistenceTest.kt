...
class RoutineOccurrencePersistenceTest {
    ...
    @Test fun v23MigrationPreservesAllExistingJournalsAndAddsOnlyEmptyOccurrenceLedger() = runBlocking {
        ...
        val oldRoot = Json.parseToJsonElement(saved.payload).jsonObject - "routineOccurrenceLedger"
        ...
        assertEquals(oldRoot, Json.parseToJsonElement(requireNotNull(dao.snapshot).payload).jsonObject - "routineOccurrenceLedger")
        assertEquals(PlannerSnapshotFormats.JSON_V24, dao.snapshot?.payloadFormat)
    }
    ...
    @Test fun everyOlderFormatRejectsInjectedLedgerAndRuntimeProofEvenNullBeforeMigration() = runBlocking {
        ...
        val currentRoot = Json.parseToJsonElement(original.payload).jsonObject
        for (version in 1..24) {
