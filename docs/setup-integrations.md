...
The macOS client may launch `codex app-server --stdio` only after verifying both
...
`account/read`. The contained runtime supports managed ChatGPT device-code login
(`chatgptDeviceCode`) only. Its Seatbelt profile permits outbound connections but
has no bind or inbound permission, so browser callback login is intentionally
unavailable.

Codex owns managed OAuth tokens and refreshes them inside a private, device-local
`CODEX_HOME`. DayWeave must not extract, back up, or sync those tokens to its
backend. Conversation history, model/reasoning choices, approvals, and streaming
will be built on the same App Server process.
...
Passing this verifier does not replace the production requirement for a native
launcher and a runtime sealed into the signed macOS app bundle.
