...
drained proof and a transaction-time registry recheck. Authoritative
interrupted-runtime recovery, pre-close settlement and reopening orchestration,
Google grant identity proof, the exclusive
...
| Secure account-deletion lifecycle | **38%** | The route-less foundation has exact cleanup manifests and bounded claims/retries, plus PostgreSQL admission ownership across runtimes. Provider work starts only after registration commits; interrupted/ambiguous records never auto-expire, and local completion alone cannot prove database drainage. Independent-runtime tests cover closure races, workspace/user overlap, interrupted work, and database loss. Fencing requires exact durable proof plus a transaction-time registry recheck. Deletion provider calls and source credential scrubbing remain absent, and all-success/legacy purge stays blocked. Next complete authoritative interrupted-runtime recovery, pending OAuth/cleanup settlement before closure, and safe reopening orchestration; then Google project-and-subject grant proof, the revocation worker with credential scrubbing, the external runtime-held restore permit, credential-only HTTP/native teardown, least-privilege roles, and backup-expiry evidence. |
