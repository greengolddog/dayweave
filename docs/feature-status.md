...
| --- | ---: | --- |
| Independent percentage, time, and named-unit progress | **90%** | Separate item-scoped authority, immutable replay/audit, both native editors and encrypted upgrades pass shared exact-value fixtures, Rust/PostgreSQL and full native regression/build gates. Strict transport, restart, conflict, late replay, failed-save rollback, privacy and unavailable-item recovery have coverage; synthetic native checks pass, including ten Android instrumented checks. Close controlled two-client/service convergence and owner-device acceptance. Parent own components remain separate from descendant counts; execution reconciliation, required-component policy and automatic completion are separate unfinished scope. |
...
| Common item model (`DOM`) | **In progress** | **80%** | Close controlled independent-progress convergence and native acceptance; complete the remaining rich fields, templates, bulk operations, audit/undo presentation, and cross-platform editing paths. |
| Hierarchy, goals, projects, routines, dependencies (`HIE`, `GOAL`, `ROU`) | **In progress** | **60%** | Close controlled project/nested-authoring, recorded-summary/independent-progress refresh and offline convergence, and hierarchy interaction acceptance; finish automatic completion/manual overrides, milestones/measures, weekly goal allocation, routine authoring/execution, and complete dependency conflict explanations. |
