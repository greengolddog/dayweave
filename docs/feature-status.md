...
Required-descendant completion now has a shared, iterative decision engine with
explicit optional branches, visible manual-policy semantics and retained
reopening provenance. The engine does not yet change authoritative lifecycle;
versioned policy commands, synchronous integration across canonical writers
and native override controls remain to be built. Bounded current-state bootstrap
is a separately covered cold-client integration prerequisite.
...
| --- | ---: | --- |
| Required-descendant parent completion and manual overrides | **10%** | Explicit rules and a pure iterative engine cover required/optional branches, manual keep-open/complete, reopening provenance, terminal ambiguity, recurrence qualification and 5,000-level completion/reopen. Historical-receipt and bounded cold-bootstrap prerequisites are covered separately. Next implement revision-bound policy commands and synchronous canonical-writer integration, preserve proposal undo/provider mappings, verify cold hydration after real completion cascades, then ship encrypted native review/override controls and full occurrence/device acceptance. No server auto-completion or native override UI is claimed yet. |
...
| Hierarchy, goals, projects, routines, dependencies (`HIE`, `GOAL`, `ROU`) | **In progress** | **60%** | Close controlled project/nested-authoring and recorded-summary convergence, independent-progress physical-device lifecycle/timing, and hierarchy interaction acceptance; finish automatic completion/manual overrides, milestones/measures, weekly goal allocation, routine authoring/execution, and complete dependency conflict explanations. |
