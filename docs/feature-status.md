...
| Route-less, default-disabled server foundation for secure personal-account deletion: detached content-free lifecycle/fence evidence, exact transition replay, a 24-hour confirmation boundary, database-level mutation fencing across the current tenant catalog and identity roots, anti-resurrection readiness/auth/bootstrap checks, an atomic personal-scope purge primitive that retains the fence, and an immutable deployment-keyed external-principal binding that rejects owner/key drift before external mutation | Part of `ID-005`, `DATA`, `REL`, and `SEC` | [deletion lifecycle migration](../server/dayweave-api/migrations/0029_account_deletion_lifecycle.sql), [external-principal migration](../server/dayweave-api/migrations/0030_account_deletion_external_principal.sql), [repository foundation](../server/dayweave-api/src/persistence/account_deletion_repository.rs), and [PostgreSQL integrity tests](../server/dayweave-api/tests/account_deletion_postgres.rs). This proves only the bounded server primitive and its fail-closed disabled state; it is not a production deletion workflow. |
...
automated PostgreSQL coverage. Each new lifecycle is also pinned to the exact
versioned deployment-keyed external principal derived for the configured owner.
It does not yet provide an HTTP operation, provider cleanup, native-client
workflow, an exclusive runtime-held external restore permit, or verified backup
erasure.
...
| Secure account-deletion lifecycle | **28%** | The committed server foundation is route-less and default-disabled; it proves detached lifecycle evidence, exact replay, database fencing, personal-scope validation, atomic local purge behavior, and immutable binding to a stable versioned deployment-keyed external identity. Wrong-owner and key-drift paths fail before external mutation. Next implement durable provider-cleanup targets/outcomes with bounded retries and an external tombstone authority whose exclusive permit remains held for runtime admission; then add the credential-only HTTP service, secure native clients, least-privilege runtime/migration roles, and backup-expiry evidence. |
