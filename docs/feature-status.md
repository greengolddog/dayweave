...
parent lifecycle or replace the still-required independent progress and
automatic-completion flows. Controlled refresh/offline convergence and owner
...
| Common item model (`DOM`) | **In progress** | **75%** | Complete the remaining rich fields, templates, bulk operations, progress modes, audit/undo presentation, and cross-platform editing paths. |
| Hierarchy, goals, projects, routines, dependencies (`HIE`, `GOAL`, `ROU`) | **In progress** | **55%** | Close controlled project/nested-authoring, recorded-summary refresh/offline convergence, and hierarchy interaction acceptance; finish independent parent progress, automatic completion/manual overrides, milestones/measures, weekly goal allocation, routine authoring/execution, and complete dependency conflict explanations. |
