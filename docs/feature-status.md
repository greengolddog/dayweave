...
two successful receipts, one sidecar and unchanged canonical/child state. This
does not close foreground polling, production device authentication/TLS or
owner-device interaction acceptance.
...
| --- | ---: | --- |
| Independent percentage, time, and named-unit progress | **95%** | Separate item-scoped authority, immutable replay/audit, both native editors and encrypted upgrades pass shared exact-value fixtures, Rust/PostgreSQL and native regression/build gates. Six controlled macOS/Android JVM phases now prove convergence through competing offline edits, genuine conflict/re-review, a lost reply and service/native restart; SQL and native checks preserve canonical state and child progress. Close foreground/reconnect/detail refresh timing, production TLS/device-auth integration and owner-device acceptance. Parent own components remain separate from descendant counts; execution reconciliation, required-component policy and automatic completion are separate unfinished scope. |
| Canonical Goals/Projects hierarchy browsing | **90%** | Both native clients show unscheduled goals and complete scoped descendant trees, retain terminal statuses, support local search/collapse and existing exact review routes, and quarantine unbound legacy data. Batch privacy/dependency lookups and graph-keyed source reuse avoid per-row full-graph rebuilds; malformed ancestry remains protected and read-only across browser and Inbox. Close controlled two-client/service refresh, native interaction/accessibility acceptance, and owner-device checks. Parent completion/progress and weekly allocations remain separate unfinished scope; project authoring now has its own checkpoint below. |
...
| Common item model (`DOM`) | **In progress** | **80%** | Close independent-progress foreground/device integration and native acceptance; complete the remaining rich fields, templates, bulk operations, audit/undo presentation, and cross-platform editing paths. |
| Hierarchy, goals, projects, routines, dependencies (`HIE`, `GOAL`, `ROU`) | **In progress** | **60%** | Close controlled project/nested-authoring and recorded-summary convergence, independent-progress foreground/device integration, and hierarchy interaction acceptance; finish automatic completion/manual overrides, milestones/measures, weekly goal allocation, routine authoring/execution, and complete dependency conflict explanations. |
