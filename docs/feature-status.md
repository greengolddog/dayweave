...
recorded success. Runtime I/O admission/draining, Google grant identity proof,
the exclusive runtime-held external restore permit, HTTP/native workflows, and
verified backup erasure remain open. PostgreSQL and Rust still block purge.
...
| Secure account-deletion lifecycle | **33%** | The committed, route-less server foundation adds provider-cleanup persistence/claim/status to the lifecycle, fencing, and external-principal binding: exact source manifests, 12-attempt bounds, 15-minute leases, a 24-hour stop for new claims/retries, and immutable historical outcome replay. Active leases may finish after that deadline. No provider calls or source credential scrubbing occur, and all-success/legacy purge remains blocked. Next add runtime guardian/distributed admission draining, Google project-and-subject grant proof, and the real revocation worker with credential scrubbing; then the external authority's runtime-held restore permit, credential-only HTTP/native teardown, least-privilege roles, and backup-expiry evidence. |
