...
| Process-local Google operation admission shared by production OAuth and sync: sticky closure, complete-operation draining, nested-call and detached-guardian ownership, cancellation-safe refresh-token custody, and exact controller/scope/deletion proof required before the low-level fence transaction or replay | Part of `ID-005`, `REL`, and `SEC` | [admission controller and tests](../server/dayweave-api/src/provider_admission.rs), [OAuth custody and tests](../server/dayweave-api/src/google_oauth/service.rs), [sync admission](../server/dayweave-api/src/google_sync/service.rs), and [fence proof tests](../server/dayweave-api/tests/account_deletion_postgres.rs). Distributed admission and pre-close/recovery orchestration remain open; deletion is still unavailable. |
...
recorded success. Production Google services now share process-local admission
and retain operation ownership through nested work, durable settlement, and
detached token custody; the low-level fence requires exact drained proof before
database access or replay. Distributed admission, pre-close settlement and
recovery/reopening orchestration, Google grant identity proof, the exclusive
...
| Secure account-deletion lifecycle | **35%** | The route-less foundation has exact manifests, bounded claims/retries, historical outcome replay, current status summaries, and process-local admission shared by Google OAuth/sync. Nested work and detached token custody retain ownership through closure; low-level fencing requires exact drained proof before DB access or replay. Deletion provider calls and source credential scrubbing remain absent, and all-success/legacy purge stays blocked. Next complete distributed admission, pending OAuth/cleanup settlement before closure, and authoritative recovery/reopening orchestration; then Google project-and-subject grant proof, the revocation worker with credential scrubbing, the external runtime-held restore permit, credential-only HTTP/native teardown, least-privilege roles, and backup-expiry evidence. |
