...
5545 `rrule` values. Custom rules use the supported finite subset, are
canonicalized and checked against the item's anchor, and generate a stable
`custom_rule` identity for every occurrence. Daily/monthly counts default to
one. Weekly count defaults to the number of selected weekdays, or one when no
...
`rolling_minutes`, `after_completion`, `rolling_month`, and `custom_rule`. The
`custom` identity variant remains decodable only for legacy compatibility and
is not generated by current scheduling.
...
move actions only for a retained legacy `identity: {"type":"custom"}`
placeholder; bounded custom recurrence with `custom_rule` identities remains
schedulable and movable.
